...
calcMov :: Element -> Board -> IO()
calcMov pieza pos = do
        putStrLn $ "Pensando..." 
        --Report winner..., missing
        let ls=(tail (fst (bestMv pieza pos 4)))
        let bm=head ls 
...
--        putStr (show square)
        ...
        calcMov pieza (newPos (change pieza) square newpos2)          

main =
     do
     putStrLn "Quieres X u O?"
     input <- getLine
     let pieza = (read input) :: Element
     calcMov Element posIni 
